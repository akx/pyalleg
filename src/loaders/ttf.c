/* PyAlleg TTF Loader
   ------------------------------------------------------------
   Heavily based on AllegroFont (c) 2001, 2002 Javier Gonzalez
   Modified by AKX (c) 2005
...
int alfont_set_font_size_old(ALFONT_FONT *f, int h) {
  ...
  // keep changing the size until the real size is not the one we want
  test_h = h;
  direction = 0;
  while (1) {
    int real_height;
...
{
	if(h==0) return ALFONT_ERROR;
	if(h<0) return alfont_set_font_size_old(f,-h);

	if(FT_Set_Char_Size(f->face, 0, h*64, 0, 0 )) return ALFONT_ERROR;
	_alfont_uncache_glyphs(f);
	f->face_h = h;
    f->real_face_h = h;
	f->face_ascender = f->face->size->metrics.ascender >> 6;
...
/*
========== PRINTF based functions are not needed ==========
